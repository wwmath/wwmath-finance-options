...
It also writes wwmath-time-machine.html: the page with the data, app and vendored
libraries inlined, so a clone opens it by double-click (file://, no web server).
"""
...
def single_file() -> str:
    """index.html with every local <script src> inlined; returns the output path."""
    ...
    assert n == 4, f"expected 4 local scripts in index.html, inlined {n}"
    html = ("<!-- GENERATED by timeline/build.py: index.html with its scripts inlined. "
            "Open it directly; no web server needed. Do not edit. -->\n" + html)
    out = os.path.join(HERE, "wwmath-time-machine.html")
...
def main() -> int:
    ...
    single = single_file()
    print(f"wrote {os.path.relpath(single, REPO)}: {os.path.getsize(single) // 1024} KiB, self-contained")
    return 0
