C     schachermayer2008/put-call-parity-target
...
C       X(4) = T
      ...
      DOUBLE PRECISION X(NIN)
      COMMON /WWIN/ X
      DOUBLE PRECISION R001, R002, R003
      DOUBLE PRECISION WWNCDF, WWNPDF, WWQUAD
C     d
      R001 = ((X(1) - X(2))/(X(3)*SQRT(X(4))))
C     C
      R002=(((X(1)-X(2))*WWNCDF(R001))+(X(3)*SQRT(X(4))*WWNPDF(R001)))
C     P
      R003=(((X(2)-X(1))*WWNCDF((-R001)))+(X(3)*SQRT(X(4))*WWNPDF(R001))
     &)
      WWEVAL = (R002 - R003)
      RETURN
      END
